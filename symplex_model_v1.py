...
class Model:
    def __init__(self) -> None:
        ...
        self.non_negativity_constraints = list()
    ...
    def constraint(self, constraint: sp.Expr):
        if len(constraint.free_symbols) == 1 and constraint.rhs == 0:
            self.non_negativity_constraints.append(constraint)
        else:
    ...
    def to_standard_form(self):
        ...
        for v in self.variables:
            positive = False
            for c in self.non_negativity_constraints:
                if c.has(v):
                    positive = True
            if not positive:
                print(v)
                self = self.substitute(v)
                self.show()
        tmp = self
        ...
        for i in range(len(tmp.non_negativity_constraints)):
            new_c = 0
            c = tmp.non_negativity_constraints[i]
    ...
    def to_slack_form(self):
        ...
        constraints = []
        slack_model.non_negativity_constraints = self.non_negativity_constraints
        for i in range(len(self.constraints)):
            c = self.constraints[i]
            constraints.append(Equation(c.lhs + sp.Symbol(f"s{i+1}"), c.rhs))
            slack_model.variable(sp.Symbol(f"s{i+1}"))
            slack_model.non_negativity_constraints.append(sp.parse_expr(f"s{i+1} >= 0"))
    ...
    def generate_aux_problem(self):
        ...
        for i in range(len(self.variables)):
            aux_problem.variable(self.variables[i])
            aux_problem.constraint(sp.parse_expr(f"{self.variables[i]}>=0"))
            
        for i in range(len(self.constraints)):
            lhs = self.constraints[i].lhs
            ...
            duplicate = False
            for j in range(len(aux_problem.constraints)):
                if aux_problem.constraints[j].lhs == lhs and aux_problem.constraints[j].rhs == rhs:
                    duplicate = True
        ...
        of = 0
        for i in range(len(aux_problem.constraints)):
            aux_problem.constraint(sp.parse_expr(f"u{i+1}>=0"))
            aux_problem.constraints[i].lhs += sp.Symbol(f"u{i+1}")
    ...
    def substitute(self, v):
        ...
        new_non_negative_constraints = self.non_negativity_constraints
        ...
        new.non_negativity_constraints = new_non_negative_constraints
    ...
    def rename_variable(self, old: sp.Symbol, new: sp.Symbol) -> None:
        ...
        for i in range(len(self.non_negativity_constraints)):
            self.non_negativity_constraints[i] = self.non_negativity_constraints[i].subs(
                old, new)
    ...
    def show(self):
        ...
        for i in self.non_negativity_constraints:
            print("\t\t", i)
